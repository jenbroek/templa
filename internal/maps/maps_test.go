...
func TestMerge(t *testing.T) {
	...
	RunTestCases(
		...
		map[string]*testCase{
			...
			"Overwrites value if same primitive type": &testCase{
				dst:   map[string]any{"foo": "1"},
				src:   map[string]any{"foo": "2"},
				want:  map[string]any{"foo": "2"},
				panic: false,
			},
			"Overwrites value if same primitive pointer type": &testCase{
			...
			"Merges maps if assignable": &testCase{
			...
			"Merges maps deeply if assignable": &testCase{
